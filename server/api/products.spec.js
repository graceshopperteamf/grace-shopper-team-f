...
describe('Product routes', () => {
    ...
    describe('Product /api/products', () => {
        ...
        beforeEach(async () => {
            ...
            let testProduct = [
                { title: 'Grace', price: 1, inventoryQuantity: 10 },
                { title: 'Shopper', price: 1, inventoryQuantity: 10 },
                { title: 'FSA', price: 1, inventoryQuantity: 10 },
            ];
                ...
                it('sends all products', () => {
                    request(app)
                        ...
                        .then((res) => {
                            ...
                            expect(
                                res.body.some(
                                    (product) => testProduct[0].title === 'Cody'
                                )
                            ).to.equal(true);
                            expect(
                                res.body.some(
                                    (product) => testProduct[1].title === 'Doug'
                                )
                            ).to.equal(true);
                            expect(
                                res.body.some(
                                    (product) =>
                                        testProduct[2].title === 'Penny'
                                )
                            ).to.equal(true);
                        });
                });
                it('sends a 404 if not found', () => {
                    return request(app).get(`/api/pugs/20`).expect(404);
                });
            });
        });
    });
